...
import apollocaffe
from apollocaffe.layers import NumpyData, LstmUnit, Concat, InnerProduct, Filler, Softmax
...
def evaluate(config, test_data):
    ...
    for idx in xrange(len(test_data['labels'])):
        ...
        with open('tmp/'+label.replace('.jpg', '.txt').replace('.jpeg', '.txt').replace('.png', '.txt'),'w') as ff: ff.write('%0.5f\n'%np.array(predictions).mean())
